function [PLdata , APDdata , MPDdata , PLvoip , APDvoip , MPDvoip , TT] = Simulator3A(lambda,C,f,P,n,b)
% INPUT PARAMETERS:
...
EventList = [ARRIVAL, tmp, GeneratePacketSize(), tmp, DATA]; % last column represents the type of the packet (0 for data and 1 for voip)
...
while (TRANSMITTEDPACKETS_D + TRANSMITTEDPACKETS_V) < P               % Stopping criterium
    ...
    switch Event
        ...
        case DEPARTURE                     % If first event is a DEPARTURE
            prob_no_errors = (1-b)^(PacketSize*8);
            if Type == DATA     % Data packet
                if rand() < prob_no_errors
