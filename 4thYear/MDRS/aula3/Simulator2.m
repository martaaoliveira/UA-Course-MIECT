...
while TRANSMITTEDPACKETS<P               % Stopping criterium
    ...
    Event= EventList(1,1);               % Get first event and (arrival or departure)
    Clock= EventList(1,2);               %   and
    PacketSize= EventList(1,3);          %   associated
    ArrivalInstant= EventList(1,4);      %   parameters.
    EventList(1,:)= [];    % Eliminate first event - so fica com os eventos futuros
    ...
    switch Event
        ...
        case DEPARTURE  % If first event is a DEPARTURE
           prob_no_error=(1-b)^(PacketSize*8);
            if(rand()<prob_no_error)
