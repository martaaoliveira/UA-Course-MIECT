...
# this function handles individual packets from the network traffic. It keeps track of various statistics related to the packets, including upload and download traffic based on client and service network IP addresses.
# It records data such as timestamp, source IP, destination IP, packet length, and the output is written to a file.
...
# The script processes different types of input files depending on the specified format:
# If the format is 1 or 2, it reads the file line by line and extracts packet information. Format 1 corresponds to a custom format, and format 2 is for tshark format.
# If the format is 3 (pcap format), it uses pyshark to capture packets and extract packet information.
