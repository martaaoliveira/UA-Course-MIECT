...
def pkt_callback(pkt):
    ...
    if (IPAddress(pkt.ip.src) in scnets and IPAddress(pkt.ip.dst) in ssnets) or (IPAddress(pkt.ip.src) in ssnets and IPAddress(pkt.ip.dst) in scnets):
        ...
        npkts=npkts+1
        if pkt.ip.proto=='17':
            print('{} IP packet from {} to {} (UDP:{}:{}) {}'.format(pkt.sniff_timestamp,pkt.ip.src,pkt.ip.dst,pkt.udp.srcport,pkt.udp.dstport,pkt.ip.len))
        elif pkt.ip.proto=='6':
            print('{} IP packet from {} to {} (TCP:{}:{}) {}'.format(pkt.sniff_timestamp,pkt.ip.src,pkt.ip.dst,pkt.tcp.srcport,pkt.tcp.dstport,pkt.ip.len))
        else:
